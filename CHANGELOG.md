...
- Streamable HTTP adapter dropped SSE responses after an empty keepalive preamble: the parser took the first `data:` line (an empty keepalive frame), handed handlers an empty string, and the initialize response was never relayed — gateway discovery then timed out with `register_complete: error`. Parsing now follows the SSE line protocol (multi-line `data:` frames joined, blank line dispatches, empty keepalive events skipped, every event relayed in order) via a new shared parser (`transports/sse_events.py`) also used by the SSE adapter; the 404 initialize-retry path is fixed the same way. (issue #9)
